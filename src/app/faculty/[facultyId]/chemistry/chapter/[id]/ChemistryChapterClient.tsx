...
export default function ChemistryChapterClient({
  ...
  const data = getSubjectData(facultyId, "chemistry");
  ...
  useEffect(() => {
    ...
    for (const sub of chapter.subchapters) {
      sp[sub.id] = getChapterProgress(facultyId, "chemistry", sub.id);
    }
    setSubProgress(sp);
  }, [chapter, facultyId]);
  ...
  if (!chapter) {
    return (
      <main className="pt-10 text-center">
        ...
        <Link href={`/faculty/${facultyId}/chemistry`} className="text-[var(--color-primary)] dark:text-blue-400 font-medium">
          Zpět
        </Link>
      </main>
    );
  ...
  const totalValid = countValidQuestions(getChapterQuestions(facultyId, "chemistry", chapterId));

  return (
    <main className="pt-6">
      <Link
        href={`/faculty/${facultyId}/chemistry`}
      ...
      <Link
        href={`/faculty/${facultyId}/chemistry/quiz?chapter=${chapterId}`}
        ...
        <div className="space-y-2.5">
          {chapter.subchapters.map((sub) => {
            ...
            return (
              <Link
                key={sub.id}
                href={`/faculty/${facultyId}/chemistry/quiz?chapter=${chapterId}&sub=${sub.id}`}
