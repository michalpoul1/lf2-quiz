...
export default function ChemistryChapterClient({
  ...
  if (!chapter) {
    return (
      <main className="pt-10 text-center">
        ...
        <Link href="/chemistry" className="text-[var(--color-primary)] dark:text-blue-400 font-medium">
          Zpět
        </Link>
      </main>
    );
  ...
  return (
    <main className="pt-6">
      ...
      <Link
        href={`/chemistry/quiz?chapter=${chapterId}`}
        ...
        <div className="space-y-2.5">
          {chapter.subchapters.map((sub) => {
            ...
            return (
              <Link
                key={sub.id}
                href={`/chemistry/quiz?chapter=${chapterId}&sub=${sub.id}`}
